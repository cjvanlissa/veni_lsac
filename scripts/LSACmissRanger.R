...
#subset
lsac12<-subset(lsac12, select = colnames(lsac12)[colnames(lsac12) %in% selected_variables$Variable.Name])
...
#from lsac14 and lsac16 only the DV is needed (SDQ emotional problems scale)
...
lsac<-merge(lsac1214, lsac16, by = "hicid",all.x = TRUE, all.y = TRUE)
